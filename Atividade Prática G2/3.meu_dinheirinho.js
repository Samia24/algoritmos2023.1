...
function main(){
    ...
    const sal_sem = salario_semanal(qtd_horas_semanais, valor_hora_aula, valor_h_prof)
    ...
    console.log(`\n> Valor hora/professor: R$ ${valor_h_prof.toFixed(2)}`)
    console.log(`> Salário Base Semanal: R$ ${sal_sem.toFixed(2)}`)
    console.log(`\n-- Ganhos --`)
    console.log(`\n> Salário Base Mensal: R$ ${sal_mens.toFixed(2)}`)
    console.log(`> Auxílio creche: R$ ${aux_cre.toFixed(2)} `)
    console.log(`> Ressarcimento saúde: R$ ${pla_saude.toFixed(2)}`)
    console.log(`> Auxílio combustível: R$ ${aux_combustivel.toFixed(2)}`)
    console.log(`\n> > Salário Bruto (Soma dos itens acima): R$ ${sal_bru.toFixed(2)}`)
    console.log(`\n-- Descontos --`)
    console.log(`\n> Previdência: R$ ${previdencia.toFixed(2)}`)
    console.log(`> Imposto de renda: R$ ${imposto.toFixed(2)}`)
    console.log(`\n> > Total de descontos (soma dos iten acima): R$ ${sal_des.toFixed(2)}`)
    console.log(`\n-- Salário Líquido (Ganhos - Descontos) --`)
    console.log(`> R$ ${sal_liq.toFixed(2)}`)
...
function valor_experiencia(tempo_exp, valor_qualificacao){
    if (tempo_exp > 6){
        let tempo = Math.trunc(tempo_exp / 7)
        let valor_exp = (0.05 * tempo) * valor_qualificacao
        return valor_exp
    }else{
        let valor_exp = 0
        return valor_exp
...
function salario_semanal(qtd_horas_semanais, valor_hora_aula, valor_prof){
    let sal_semanal = (qtd_horas_semanais * valor_hora_aula) + valor_prof
    return sal_semanal
}
...
function plano(plano_saude){
    if (plano_saude <= 1000){
        let sal_plano = (0.5 * plano_saude)
        return sal_plano
    }else{
        let sal_plano = 500
        return sal_plano
...
function salario_descontos(prev_social, imposto_renda){
    if (imposto_renda !== 'Isento'){
        let descontos = prev_social + imposto_renda
        return descontos
    }else{
        let descontos = prev_social
        return descontos
    }
